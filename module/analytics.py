...
class Analytics:
    ...
    def __check(self, count_max_events) -> None:
        while True:
            ...
            for ip in ips:
                ...
                if (count_ip_events >= count_max_events) or ((datetime.datetime.now() - self.__store[ip]['time_create']).seconds >= 300 and count_ip_events >= int(0.5 * count_max_events)):
    ...
    def __preprocessing(self, ip) -> None:
        ...
        if not self.model.check_event(list(dict(OrderedDict(sorted(d.items()))).values())):
            print(event_info)
